...
export default class Home  extends Component {
 ...
 handleAutocompleteAddEvent(obj) {
   let id = (+ new Date() + Math.floor(Math.random() * 999999)).toString(36);
   let tax = parseFloat(obj.mrp) * parseFloat(obj.tax)/100;
   let amount= parseFloat(obj.mrp) +tax ;
   let product = {
     id:id,
     pid:obj._id,
...
let products = this.state.products.slice();
 ...
 windowPrint(){
  ...
  render(){
    ...
    let filterProducts=this.props.masterproducts.filter(
       (product)=>{
     return (product.productName.toLowerCase().indexOf(this.state.autocompletetext.toLowerCase()) !==-1);
   }
);
    return(
      <div>
                     ...
                     renderItem={(item, isHighlighted) =>
                       {
                         ...
                         <div className="myselectdiv-subdiv">{item.productName}</div>
                         <div className="myselectdiv-subdiv">{item.mrp}Rs.</div>
                         </div>);
                        }}
